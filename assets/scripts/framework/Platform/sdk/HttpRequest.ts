...
export class HttpRequest {
    ...
    private sendRequest(name: string,
        ...
        xhr.onerror = () => {
            console.log("xhr.onerror", xhr.readyState)
            this.deleteCache(newUrl);
        ...
        xhr.onreadystatechange = () => {
            ...
            if (xhr.status == 200) {
                if (completeCallback) {
                    if (responseType == 'arraybuffer') {
                        ...
                        try {
                            ...
                            var jsonData: any = JSON.parse(xhr.response);
                            console.log("xhr.response jsonData", jsonData)
                            console.log("xhr.response completeCallback", completeCallback)
                            if (completeCallback) {
                ...
                // 其他状态码
                errorCallback && errorCallback(xhr.response);
            }
        };
        ...
        if (params == null || params == "") {
            ...
            let reqJsonStr = JSON.stringify(params)
            console.log(url, reqJsonStr);
            xhr.send(reqJsonStr);
        }
    }
